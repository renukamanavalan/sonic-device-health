...
import (
	...
	//"gocode/src/pluginmgr/pluginmgr_common"
	//"gocode/src/plugins/plugins_common"
	//"gocode/src/plugins/plugins_files"
	//"io/ioutil"
	//"log/syslog"
	"os"
	"path/filepath"
...
func TestReadProcsConf(t *testing.T) {
	...
	t.Run("proc ID invalid", func(t *testing.T) {
		...
		assert.NotNil(t, err)
		assert.Regexp(t, regexp.MustCompile(`Procs: .*: Failed to get config for proc \(\w+\)`), err.Error())
	})
}
