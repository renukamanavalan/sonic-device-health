...
import (
	...
	. "go/src/vendors/sonic/client/dbclient"
)
...
func Test_GetInterfaceCounters_ReturnsAllCountersSuccessfuly(t *testing.T) {
	...
	(mockRedisProvider).On(HGetAllMethod, 2, COUNTERS_PORT_NAME_MAP).Return(newMap, nil)
...
func Test_GetRedisConnectionForDatabase_ReturnsConnectionSuccessfuly(t *testing.T) {
	redisClient1 := GetRedisConnectionForDatabase(2)
	redisClient2 := GetRedisConnectionForDatabase(2)
