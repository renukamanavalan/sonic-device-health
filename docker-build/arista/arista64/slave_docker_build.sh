...
docker inspect --type image arista64-lom-slave-bullseye:1234 &> /dev/null; RET=$?
if [[ ${RET} -ne 0 ]];then
    ...
    docker build --no-cache \
            -t arista64-lom-slave-bullseye:1234 \
             lom-slave-bullseye 2>&1 | tee lom-slave-bullseye/arista64-lom-slave-bullseye.log
    docker inspect --type image arista64-lom-slave-bullseye:1234 &> /dev/null; RET=$?
    if [[ ${RET} -ne 0 ]];then
    ...
    echo "arista64-lom-slave-bullseye:1234 is ready"
fi

docker inspect --type image arista64-lom-slave-bullseye-admin:1234 &> /dev/null; RET=$?
if [[ ${RET} -ne 0 ]];then
    docker build --no-cache \
        ...
        -t arista64-lom-slave-bullseye-admin:1234 \
        -f lom-slave-bullseye/Dockerfile.user \
        lom-slave-bullseye 2>&1 | tee lom-slave-bullseye/arista64-lom-slave-bullseye-admin.log
else
    echo "arista64-lom-slave-bullseye-admin:1234 is ready"
fi
