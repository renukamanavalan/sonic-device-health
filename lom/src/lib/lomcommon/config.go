...
func (p *ConfigMgr_t) readActionsConf(actionFilePaths []string) error {
	allActionsConf := ActionsConfigList_t{}
	for _, actionFilePath := range actionFilePaths {
		actionConfig := make(map[string]ActionCfg_t)
		jsonFile, err := os.Open(actionFilePath)
		...
		if byteValue, err := io.ReadAll(jsonFile); err != nil {
			return err
		} else if err := json.Unmarshal(byteValue, &actionConfig); err != nil {
			return err
		}

		if len(actionConfig) > 1 {
			return LogError("ActionConf file contains more than one configuration. %s", actionFilePath)
		}

		for actionConfigKey, actionConfigValue := range actionConfig {
			allActionsConf[actionConfigKey] = actionConfigValue
		}
	}
	p.actionsConfig = allActionsConf
	return nil
}
...
func InitConfigPath(path string) error {
	...
	allActionConfigFilePaths, err := GetAllActionConfigFilePaths(path)
	if err != nil {
		return err
	}
	...
	cfgFiles := &ConfigFiles_t{
		...
		ActionsFl:  allActionConfigFilePaths,
...
func GetAllActionConfigFilePaths(path string) ([]string, error) {
	actionsFolderPath := filepath.Join(path, ACTIONS_CONF_FOLDER)
	files, err := os.ReadDir(actionsFolderPath)
	if err != nil {
		return nil, err
	}
	...
	for _, file := range files {
		allFilePaths = append(allFilePaths, filepath.Join(actionsFolderPath, file.Name()))
	}
